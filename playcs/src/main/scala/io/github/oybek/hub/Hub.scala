...
object Hub:
  def create[F[_]: MonadThrow: ContextLogger, G[_]: Monad]
    ...
    new Hub[F]:
      ...
      private def handleNewCommand(chatId: ChatIntId, map: String): Context[F[List[Reaction]]] = {
        for
          hlds <- OptionT(hldsPool.find(chatId.id))
          _    <- OptionT.liftF(hlds.changeLevel(map))
        yield List(SendText(chatId, "You already got the server, just changing a map"))
      } orElse {
        for
      ...
      private def sendConsole(chatId: ChatIntId, console: Hlds[F], password: String): SendText =
